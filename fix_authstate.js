...
files.forEach(f => {
    ...
    c = c.replace(/private async Task ProcessSingleDelete\(int currentId\)\s*\{([\s\S]*?)\}/g, (match, inner) => {
        let fixedInner = inner.replace(/authState\.User/g, '(await AuthStateProvider.GetAuthenticationStateAsync()).User');
        return `private async Task ProcessSingleDelete(int currentId)\n    {${fixedInner}}`;
    });
    fs.writeFileSync(f, c, 'utf8');
    console.log("Fixed authState in " + f);
});
